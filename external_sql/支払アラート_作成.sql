-- 「支払アラート」作成
-- 2022/09  Ver 1.0 初版
-- 2024/11  Ver 1.1 「利用サービス」の代わりに「本人特定契約」を使用する
--------------------------------------------------------------------------
...
SELECT DISTINCT IRIM_SQSB_NO           -- 依頼元請求識別番号
               ...
               ,SIK_SIK_TTNO           -- 請求_請求特定番号
FROM [omni].[omni_ods_ciriuscf_trn_uncollected] -- 未収
...
AND   SIK_SIK_SVC_SHBT not in('11','14','15','21','13') -- (11,14,15(一括),21(分割),13(郵送一括))ではない
;
...
INSERT [omni].[omni_ods_ma_trn_uncollected_gas_temp]        -- ②ガス契約未収
SELECT   temp.*                                             -- ①の全項目
       , CONVERT(varchar,gas.SYO_KYO_TRNO)                  -- A.使用者契約お客さま登録番号（4xに対する3x）
       , gas.SIH_HUHU_SHBT                                  -- A.支払方法種別 
       , CONVERT(varchar,gas.SHSY_KYO_TRNO)                 -- A.支払者契約お客さま登録番号（2xに対する3x）
...
AND   gas.SIK_SVC_SHBT NOT IN('11','14','15','21','13')     -- (11,14,15(一括),21(分割),13(郵送一括:オーナー))ではない
;
...
WHERE EXISTS(SELECT 1 FROM [omni].[omni_ods_ma_trn_paymentalert_history] pah WHERE pah.SIK_NO=pa.SIK_NO and pah.SIK_KRNO=pa.SIK_KRNO)
;
...
WHERE pa.EXISTING_FLAG=1    -- 既存フラグ=1について更新する
;
