...
class DockerE2EConnection:
    ...
    def _execute_enhanced_local_simulation(self, pipeline_name: str, parameters: Dict) -> Dict:
        ...
        try:
            ...
            elif pipeline_name == "point_grant_email":
        ...
        try:
            ...
            for row in source_data:
                marketing_row = {
                    ...
                    "data_source": "marketing_pipeline"
                }
    ...
    def _simulate_point_grant_email_pipeline(self, execution_id: str, parameters: Dict) -> Dict:
        ...
        try:
            # point_grant_emailテーブルからPENDINGステータスのデータを取得
            source_data = self.execute_query(
                "SELECT * FROM [dbo].[point_grant_email] WHERE status = 'PENDING'"
            )
            rows_processed = len(source_data)
            
            # メール送信シミュレーション（ステータス更新）
            if rows_processed > 0:
                self.execute_query(
                    "UPDATE [dbo].[point_grant_email] SET status = 'SENT', processed_date = GETDATE() WHERE status = 'PENDING'"
                )
