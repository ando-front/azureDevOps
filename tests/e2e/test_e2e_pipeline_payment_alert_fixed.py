"""
E2Eテスト: pi_Send_PaymentAlert パイプライン（支払いアラート・CSV・SFTP送信）

このテストは、pi_Send_PaymentAlert パイプラインの実際の機能を検証します。
パイプラインは以下の処理を実行します：
1. DAM-DBから支払いアラートデータを抽出
2. CSV.gz形式でBlob Storageに出力
3. SFMCにSFTP送信

【実際の実装】このパイプラインは単純なデータ転送処理のみを行います：
- at_CreateCSV_PaymentAlert: SQLクエリ実行 → CSV.gz生成
- at_SendSftp_PaymentAlert: Blob Storage → SFTP転送
...
class TestPipelinePaymentAlert:
    """pi_Send_PaymentAlert パイプライン CSV・SFTP送信E2Eテストクラス"""
    ...
    def setup_method(self):
        ...
        logger.info(f"支払いアラート・CSV・SFTP送信E2Eテスト開始: {self.PIPELINE_NAME} - {self.start_time}")
        
    def teardown_method(self):
        """テストメソッド終了処理"""
        end_time = datetime.now()
        duration = end_time - self.start_time
        logger.info(f"支払いアラート・CSV・SFTP送信E2Eテスト完了: {self.PIPELINE_NAME} - 実行時間: {duration}")
    ...
    def test_pipeline_execution_success(self, helper, pipeline_run_id):
        ...
        logger.info("パイプライン実行成功テスト開始 (PA-001)")
    ...
    def test_performance_validation(self, helper, pipeline_run_id):
        ...
        logger.info(f"パフォーマンス検証完了 (PA-006): 実行時間={execution_time}秒")

    def test_error_handling_validation(self, helper):
        """エラーハンドリング検証テスト (PA-007: エラーハンドリング確認)"""
        logger.info("エラーハンドリング検証テスト開始 (PA-007)")
        ...
        try:
            ...
            logger.info("エラーハンドリング検証完了 (PA-007)")
    ...
    def test_security_validation(self, helper, pipeline_run_id):
        ...
        logger.info("セキュリティ検証テスト開始 (PA-008)")
    ...
    def test_data_consistency_validation(self, helper, pipeline_run_id):
        ...
        logger.info("データ整合性検証テスト開始 (PA-009)")
        ...
        try:
            ...
            logger.info(f"データ整合性検証完了 (PA-009): csv={csv_count}行")
    ...
    def test_file_format_validation(self, helper, pipeline_run_id):
        ...
        logger.info("ファイル形式検証テスト開始 (PA-010)")
