"""
E2Eテスト: pi_Copy_marketing_client_dm パイプライン（460列完全テスト）

このテストは、pi_Copy_marketing_client_dm パイプラインの実際の460列構造を完全に検証します。
パイプラインは以下の処理を実行します：
1. Marketingスキーマの顧客DMから作業テーブル（omni_temp）への全量コピー
2. 作業テーブルから本テーブル（omni.顧客DM）への全量コピー

【実際の460列構造】このパイプラインは以下を含む460列の顧客データ構造を処理します：
...
class TestPipelineMarketingClientDMComprehensive:
    """pi_Copy_marketing_client_dm パイプライン 460列包括的E2Eテストクラス"""
    ...
    CRITICAL_COLUMN_GROUPS = {
        "core_client": {
            "columns": ["CLIENT_KEY_AX"],  # 実際に存在するコア顧客情報のみ
            "description": "顧客コア情報"
        },
        "gas_meter": {
            "patterns": ["LIV0EU_*"],
    ...
    def setup_class(self):
        ...
        logger.info(f"460列包括的E2Eテスト開始: {self.PIPELINE_NAME} - {self.start_time}")
        
    def teardown_class(self):
        """テストクラス終了処理"""
        end_time = datetime.now()
        duration = end_time - self.start_time
        logger.info(f"460列包括的E2Eテスト完了: {self.PIPELINE_NAME} - 実行時間: {duration}")
    ...
    def _validate_comprehensive_column_structure(self, helper, table_name: str, table_type: str):
        ...
        try:
            ...
            logger.info(f"460列構造検証完了: {table_name}")
    ...
    def _validate_column_group(self, helper, group_name: str, group_info: Dict[str, Any]):
        ...
        try:
            ...
            # 460列全てが常に存在するとは限らない（スキーマ変更やNULL設定列）
    ...
    def test_performance_validation(self, helper, pipeline_run_id):
        ...
        logger.info(f"パフォーマンス検証完了: 実行時間={execution_time}秒, 処理レコード={processed_rows}件, 460列構造")
