...
-- 
-- 【データ概要】
-- LIM精算用故障修理データから、実行日の21日前以降に作業完了したガス機器修理・
-- 水まわり修理の実績があるお客様を抽出し、MA向けに出力する。
-- 
-- 【出力カラム構成】 8カラム
-- Bx                         : 利用サービスマスタのBx
-- mTGID                      : myTGID
...
-- OUTPUT_DATETIME            : IF出力日時
-- 
-- 【ビジネスロジック】
-- Step1: 中間テーブルに作業完了データを集約
--   - 実行日-21日以降の作業完了データを抽出
--   - ガス機器修理(作業対象区分='11')、水まわり修理(作業対象区分='1A')をフラグ化
--   - ガスメータ設置場所番号、作業完了年月日でグループ化
-- 
-- Step2: 対象外レコードの除外
--   - 利用サービステーブルで1つのガスメータ設置場所番号に複数のBxが紐づく場合を除外
-- 
-- Step3: MA向け最終出力
--   - 利用サービステーブルとJOINしてBx、myTGID、INDEX_IDを取得
--   - 同一キーで複数INDEX_IDがある場合はOUTPUT_DATE降順で最新を採用
-- 
-- 【作成者】: 自動外部化処理
-- 【作成日】: 2025/01/16
-- 【外部参照】: src/dev/arm_template/linkedTemplates/ArmTemplate_3.json (line:2215)
--------------------------------------------------------------------------
...
SELECT
    GMT_SET_NO                                                                                           --ガスメータ設置場所番号
     ...
     , MAX(CASE WHEN WORK_SUBJECT_CATEGORY_CODE = '1A' THEN '1'ELSE '0' END) AS WET_AREA_REPAIR_FLAG
--水まわり修理フラグ
FROM [omni].[omni_ods_livalit_trn_lim_settlement_breakdown_repair]
--ＬＩＭ精算用故障修理データ
WHERE  GMT_SET_NO <> '00000000000' --ガスメータ設置場所番号が無い(00000000000)レコード以外抽出する
    ...
    AND WORK_SUBJECT_CATEGORY_CODE IN ('11','1A')
--作業対象区分コードが11(一般機器)か1A(水まわり修理(加入なし))を抽出する
...
FROM(
    SELECT
        us.Bx                           --Bx
          ...
          , ROW_NUMBER() OVER(PARTITION BY us.Bx,rep.GMT_SET_NO,rep.WORK_COMPLETION_DATE  ORDER BY OUTPUT_DATE desc)  as rownum
    -- 同一「Bx,ガスメータ設置場所番号,作業完了年月日」に異なる「INDEX_ID」が存在する場合は「OUTPUT_DATE」が新しいレコードを抽出する
