/*
開栓後の支払方法のご案内出力クエリ
パイプライン: pi_OpeningPaymentGuide
目的: ガス開栓者に対する支払方法案内のためのデータ抽出（開栓者全量連携）
...
抽出条件:
- TG小売り（TENKEN_JUTAKU_KBN='1'）
- 代替・新設（KAISEN_JIYU_CD in ('11','12')）
- 実行日から20日前～5日前の作業
- 正常・フォロー済（ERROR_FOLLOW_JOTAI_CD in('0','2')）

外部化前の場所: ARMTemplateForFactory.json line 5096
作成日: 2024-12-19
*/

-- @name: opening_payment_guide_main
-- @description: 開栓後の支払方法のご案内データの抽出とCSV出力
...
SET @from20days=format(dateadd(day,-20, @today_jst),'yyyyMMdd');
-- 実行日JSTから20日前

DECLARE @to5days varchar(8);
SET @to5days=format(dateadd(day,-5,@today_jst),'yyyyMMdd');
-- 実行日JSTから5日前
...
WHERE TENKEN_JUTAKU_KBN='1' -- TG小売り
    ...
    AND ERROR_FOLLOW_JOTAI_CD in('0','2')
-- 0：正常,2:フォロー済
;
...
SELECT
    ...
    , NULL as SIH_HUHU_SHBT
-- A.SIH_HUHU_SHBT as SIH_HUHU_SHBT    -- 支払方法種別　...出力から無くし固定のNULLとする
...
SELECT
    ...
    , format(@today_jst, 'yyyy/MM/dd HH:mm:ss') as OUTPUT_DATETIME
-- IF出力日時
FROM(
    SELECT
        A.Bx
        , A.INDEX_ID
        , temp2.*
        , ROW_NUMBER() OVER(PARTITION BY Bx,ANKEN_NO,GASMETER_SETTI_BASYO_NO,SAGYO_YMD ORDER BY OUTPUT_DATE desc) as rown
    -- INDEX_IDを除くカラムごとのOUTPUT_DATEの降順
