...
-- Source: src/dev/arm_template/linkedTemplates/ArmTemplate_3.json
-- Activity: at_CreateCSV_ElectricityContractThanks (pi_Send_ElectricityContractThanks)
-- Created: 2024-12-31
--
-- Purpose: 電気契約の新規開始に対する感謝シナリオ用データを抽出
-- 
-- Column Structure (13 columns):
-- 1. MTGID - mTGID (本人特定契約から取得)
-- 2. KEY_1X - ガスメータ設置場所番号
-- 3. KEY_3X - お客さま登録番号
-- 4. GAS_4X - ガス使用契約番号
-- 5. EL_4X - 電力使用契約番号
-- 6. KEY_8X - カスタマ番号
-- 7. SUPPLY_POINT_ID - 供給地点特定番号
-- 8. SA_ID - 電力契約番号
-- 9. START_DATE - 使用開始日
-- 10. APPLICATION_DT - 申込日
-- 11. SA_TYPE_CD - 契約タイプ (2024/10 追加)
-- 12. TRKSBTCD - 種別コード (2024/10 追加)
-- 13. OUTPUT_DATETIME - IF出力日時
--
-- Business Logic:
-- - 電力CIS契約情報と指図情報を結合
-- - 実行日-1日の新規契約を対象
-- - 供給地点特定番号ごとに最初の契約のみ抽出
-- - 履歴テーブルに記録後、MA向けに出力 (実行日-30日～-25日の範囲)
-- - 2024/10: 出力カラム「契約タイプ」追加、mTGID取得を「本人特定契約」に変更
--------------------------------------------------------------------------
...
FROM [omni].[omni_ods_epcis_trn_contract] c -- 電力ＣＩＳ契約情報
    ...
    ON c.SA_ID=o.SA_ID -- 「電力契約番号」が一致
        AND o.OO_STATUS_FLG='800' AND o.OO_TYPE_CD='ESST'
-- 開始作業完了
...
WHERE c.UPD_DIVISION='A' -- 更新区分=A:追加
    ...
    FROM (
            SELECT
            count(distinct ct.SA_ID) as count
        -- 「電力契約番号」の数
        FROM [omni].[omni_ods_epcis_trn_contract] ct
        -- 電力ＣＩＳ契約情報
...
SELECT distinct
	...
	, @runYMD
FROM [omni].[omni_ods_ma_trn_new_electricity_contract_temp] ct
-- 中間①
WHERE not EXISTS(
    SELECT 1
FROM [omni].[omni_ods_ma_trn_electricity_thanks_history] th
-- 電気契約Thanks履歴
...
------------------------------------------------------------------
--④ MA向けに出力する
DECLARE @targetDate30 varchar(8);
-- 抽出対象日=実行日-30日
SET @targetDate30=format(DATEADD(DAY,-30,@today_jst),'yyyyMMdd');

DECLARE @targetDate25 varchar(8);
-- 抽出対象日=実行日-25日
